...
def prompt_for_admin_password
  password = ask('Password [antgrape123]: ') do |q|
    q.echo = false
    q.validate = /^(|.{5,40})$/
    q.responses[:not_valid] = "Invalid password. Must be at least 5 characters long."
    q.whitespace = :strip
  end
  password = "antgrape123" if password.blank?
  password
end

def prompt_for_admin_email
  email = ask('Email [antgrape@example.com]: ') do |q|
    q.echo = true
    q.whitespace = :strip
  end
  email = "antgrape@example.com" if email.blank?
  email
end

def create_admin_user
  if ENV['AUTO_ACCEPT']
    password =  "antgrape123"
    email =  "antgrape@example.com"
  else
